...
const instrumentLayer = {
  ...
  rows: [
    {
      cluster: "Capture",
      instrument: "Wispr Flow",
      role: "Capture thinking at the speed of talking.",
      status: "active" as const,
    },
    {
      cluster: "Create",
      instrument: "Nano Banana",
      role: "Think visually and prototype fast.",
      status: "active" as const,
    },
    {
      cluster: "Create",
      instrument: "Veo 3 · Kling · Higgsfield",
      role: "Motion concepts and demos.",
      status: "active" as const,
    },
    {
      cluster: "Create",
      instrument: "Gamma · Gemini in G-Suite",
      role: "Turn thinking into something I can present.",
      status: "active" as const,
    },
    {
      cluster: "Build",
      instrument: "VSCode · Codex · AntiGravity",
      role: "Where I actually build.",
      status: "active" as const,
    },
    {
      cluster: "Connect",
    ...
    },
    {
      cluster: "Watch",
      instrument: "Qwen 3.5",
      role: "Models that run privately, on my own compute.",
      status: "watching" as const,
    },
    {
      cluster: "Watch",
      instrument: "DeepSeek · Kimi",
      role: "Watching the China frontier.",
      status: "watching" as const,
    },
  ],
  exploringNext: {
    label: "Actively exploring next",
    items: [
      {
        instrument: "Kimi Swarms",
        role: "Multi-agent swarm execution.",
        principle:
          "Evaluating whether swarms beat single agents on real tasks — not yet deployed.",
      },
    ],
  },
};

const instrumentStatusStyles: Record<"active" | "watching", string> = {
  active: "bg-emerald-50 text-emerald-800 ring-emerald-200",
  watching: "bg-amber-50 text-amber-800 ring-amber-200",
};
...
export default function Home() {
  return (
    <main className="min-h-screen bg-[#f8fafd]">
        ...
        <div className="rounded-[28px] border border-[#dfe3eb] bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-6 border-t border-[#dfe3eb] pt-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1.5 rounded-full border border-[#dadce0] bg-[#f1f3f4] px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-[0.12em] text-[#5f6368]">
      ...
      <section className="mx-auto w-full max-w-6xl px-4 pb-6 sm:px-5 lg:px-6">
        <details className="group overflow-hidden rounded-[24px] border border-[#dfe3eb] bg-white shadow-sm">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-5 transition hover:bg-[#f8fafd] sm:p-6 [&::-webkit-details-marker]:hidden">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#1a73e8]">
                {instrumentLayer.kicker}
              </p>
              <h2 className="mt-1 text-xl font-semibold tracking-[-0.025em] text-[#202124] sm:text-2xl">
                {instrumentLayer.heading}
              </h2>
            </div>
            <span className="flex shrink-0 items-center gap-2 rounded-full border border-[#dadce0] bg-white px-3 py-1.5 text-[11px] font-bold text-[#5f6368] transition group-hover:border-[#c6dafc] group-hover:text-[#1a73e8]">
          ...
          <div className="border-t border-[#dfe3eb] p-5 sm:p-6">
            <p className="max-w-3xl text-sm leading-6 text-[#5f6368]">
              {instrumentLayer.subheading}
            </p>

            <p className="mb-2 mt-4 text-[10px] font-medium italic text-[#5f6368] sm:hidden">
              ← swipe to see all columns →
            </p>
            <div className="mt-2 overflow-x-auto rounded-2xl border border-[#dadce0] bg-white sm:mt-4">
              <table className="w-full min-w-[640px] text-left text-sm">
                <thead>
                  <tr className="border-b border-[#dadce0] bg-[#f8fafd]">
                    {["Stage", "Instrument", "Role"].map((col) => (
                      <th
                        key={col}
                        scope="col"
                        className="px-4 py-3 text-[10px] font-bold uppercase tracking-[0.16em] text-[#5f6368]"
                      >
                        {col}
                      </th>
                    ))}
                ...
                <tbody>
                  {instrumentLayer.rows.map((row, i) => (
                    <tr
                      key={`${row.cluster}-${row.instrument}`}
                      className={
                        i < instrumentLayer.rows.length - 1
                          ? "border-b border-[#eaecf0]"
                          : ""
                      }
                    >
                      ...
                      <td className="px-4 py-4 align-top">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="font-semibold tracking-[-0.01em] text-[#202124]">
                            {row.instrument}
                          </span>
                          <span
                            className={`inline-flex rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.12em] ring-1 ${instrumentStatusStyles[row.status]}`}
                          >
                            {row.status}
                          </span>
                        </div>
            ...
            <div className="mt-4 rounded-2xl border border-[#d2e3fc] bg-[#e8f0fe] p-4">
              <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#1a73e8]">
                {instrumentLayer.exploringNext.label}
              </p>
              <div className="mt-2 space-y-2">
                {instrumentLayer.exploringNext.items.map((item) => (
                  <div
                    key={item.instrument}
                    className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:gap-3"
                  >
                    <span className="shrink-0 font-semibold tracking-[-0.01em] text-[#202124]">
                      {item.instrument}
                    </span>
                    <span className="text-sm leading-6 text-[#174ea6]">
      ...
      <footer className="mx-auto w-full max-w-6xl px-4 pb-10 pt-2 sm:px-5 lg:px-6">
        <div className="flex flex-col items-center gap-3 border-t border-[#dfe3eb] pt-6">
          <div className="flex items-center gap-3">
            ...
            <a
              href="https://github.com/sachdevageetesh"
              target="_blank"
              rel="noreferrer"
              aria-label="GitHub"
              className="rounded-full border border-[#dadce0] bg-white p-2.5 text-[#5f6368] transition hover:-translate-y-0.5 hover:border-[#c6dafc] hover:text-[#1a73e8] hover:shadow-md"
